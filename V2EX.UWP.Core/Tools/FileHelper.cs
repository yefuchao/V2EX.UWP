    ...
    public class FileHelper
    {
        ...
        public async Task SaveImageAsync(WriteableBitmap image,string filename)
        {
            try
            {
                ...
                var folder = await _local_folder.CreateFolderAsync("image_cache", CreationCollisionOption.OpenIfExists);
        ...
        public async Task DeleteCacheFile()
        {
            try
            {
                StorageFolder folder = await _local_folder.GetFolderAsync("image_cache");
                if (folder != null)
                {
                    //IReadOnlyCollection<StorageFile> files = await folder.GetFileAsync(Windows.Storage.Search.CommonFileQuery.DefaultQuery);
